...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="cn-parties/cn-cn-applicants/cn-applicant/@sequence">
        <xsl:element name="{'pat:PartyBag/pat:ApplicantBag/pat:Applicant/pat:SuccessorRights/@pat:cn-applicantSequenceNumberReference': True}">
            <xsl:apply-templates select="@*|node()"/>
        </xsl:element>
    </xsl:template>
    ...
    <xsl:template match="rep-type[text() = 'cn-agent']">
    ...
    <xsl:template match="text()[text() = 'cited by cn-applicant']">
    ...
    <xsl:template match="app-type[text() = 'cn-applicant-cn-inventor']">
